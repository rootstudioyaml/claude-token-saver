...
**에이전트의 같은 실수, 두 번은 없습니다.** Sprag는 Claude Code에서 쓰는 명령줄 하네스입니다. 세션 기록을 읽어 되풀이된 실패를 규칙으로 만들어 두고 이후 모든 세션에 자동으로 불러옵니다. 단순한 작업은 더 저렴한 모델에 맡기고, 캐시와 사용량 한도 문제는 세션을 잃기 전에 알려 줍니다.

아낀 돈은 두 줄로 보여 줍니다. 라우팅과 문서 변환이 각자 한 줄씩 차지하며, 두 수치 모두 추정이 아니라 원장 기록이고, 더 많이 아낀 쪽이 위에 놓입니다. 의존성 0, 설치 한 줄.
