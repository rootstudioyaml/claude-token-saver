...
### v3.46.1 (2026-09-16)
- **설치가 정식 명령 이름을 씁니다** — `install` 이 등록하는 statusline 과 훅 명령이 모두 레거시 이름 `claude-token-saver` 로 하드코딩되어 있었습니다. 두 바이너리가 함께 배포되므로 동작에는 문제가 없었지만, 패키지가 `sprag-cli` 인데 사용자가 자기 `settings.json` 에서 자기가 적지 않은 이름을 보게 됩니다. 이제 `sprag` 로 씁니다.
