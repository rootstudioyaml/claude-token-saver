...
| Dataset | Router | gpt-5 | Note |
|---|---|---|---|
| SimpleQA | 53.8% / $0.78 | 47.9% / $57.25 | mid model beats flagship at 1/73 the cost |
| SWE-Bench | 25.0% / $2.31 | 15.8% / $24.27 | long-context repair: cheap ≥ flagship |
| AIME | 90.0% / $4.33 | 88.3% / $4.57 | |
| MMLU-Pro | 84.7% / $21.06 | 87.4% / $36.17 | flagship keeps a small edge |
| GPQA | 76.3% / $5.02 | 84.8% / $7.94 | hardest science QA stays flagship territory |
...
- It does **not** simulate the tool's actual production loop — delegating
  ...
  the routing ledger (`route-scan savings`).
...
- **Routing ledger**: production delegations are recorded one by one — the
  ...
  that actually ran, at the same token counts. `claude-token-saver
  route-scan savings` traces every dollar back to the run that earned it.
