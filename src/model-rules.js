...
export function renderModelRatchet(rules, lang = userLanguage()) {
  ...
  const lines = ko ? [
    ...
    '- **검증 가능성**: 결과가 명령 출력·테스트·exit code로 자동 판정되는 작업은',
    '  하위 티어를 우선한다. 검증이 어렵거나 틀리면 비용이 큰 작업은 위임하지 않는다.',
    ...
    '  않는다. 단, 의존 단계가 하나뿐인 짧은 구현은 분해 오버헤드가 더 크므로',
    '  메인 모델이 직접 한다. 위임한 구현이 돌아오면 메인 모델이 diff와 테스트',
    '  결과를 검증하고 나서 완료로 보고한다. 이 검증 비용까지가 위임 비용이다.',
    ...
    '- **Verifiability**: prefer the lower tier when the result is auto-checked',
    '  by command output, tests, or an exit code. Never delegate work that is',
    '  hard to verify or expensive when wrong.',
    ...
    '  model: sonnet subagent — never haiku. A short implementation with a',
    '  single dependency chain stays on the main model: decomposition overhead',
    '  outweighs the saving. When a delegated implementation comes back, the',
