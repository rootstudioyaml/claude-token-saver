...
<details>
<summary>왜 이 설계인가 — 리서치 근거 (deep-research 소스 21개 교차검증 + 로컬 실측 553 에피소드)</summary>

기존 LLM 라우팅 연구·시스템을 조사한 결과가 설계의 각 축을 결정했습니다:

- **"쉬운 것만 확실히 내리는" 보수적 강등이 미개척 지대** — RouterArena(2025) 평가에서 학술·상용 라우터 전부 오라클(90.9%)에 크게 못 미쳤고(1위 66.9), 실패 방향은 일관되게 **비싼 모델로의 과잉 라우팅**이었습니다. 오라클 기준 최상위 모델이 정말 필요한 쿼리는 20% 미만인데(routing collapse 연구), 저자 로그에선 73%가 최상위 티어에 남아 있었습니다 — 헤드룸이 큽니다.
- **난이도는 텍스트 추정이 아니라 결과(outcome)로 정의** — RouterArena는 난이도를 "42개 모델 중 몇 개가 실제로 맞혔나"로 라벨링했습니다. 우리 로그에도 결과가 이미 있습니다: 도구 에러·변경성 도구 수·에피소드 길이. 로컬 실측에서도 에러 경험률이 티어를 강하게 갈랐습니다 (T2 후보 2% / T1 8% / T0 36%).
- **임계값은 사용자 분포에서 보정** — RouteLLM의 공식 한계가 "고정 임계값은 쿼리 분포가 바뀌면 어긋난다"입니다. 그래서 출력 토큰 임계값을 최근 14일 p25/p75에서 재계산합니다(클램프 포함).
- **실패 시 승급 경로가 강등 오판 비용을 낮춤** — FrugalGPT 캐스케이드("싼 모델부터, 안 되면 위로")의 교훈. Claude Code에선 서브에이전트가 막히면 메인 모델이 이어받는 형태로 자연 구현되고, 승격 룰 텍스트에도 이 가드가 명시됩니다.
- **서브에이전트 `model:`은 Anthropic 공식 비용 통제 메커니즘** — 공식 문서가 "Haiku 등 싼 모델로 라우팅해 비용 통제"를 권장하지만, Claude Code 본체는 자동 모델 라우팅을 하지 않습니다. 사소한 턴도 세션 모델이 전체 컨텍스트를 재전송하며 처리하는 이 공백이 route-scan이 메우는 자리입니다.
- **학습형 분류기(BERT/임베딩)는 의도적으로 배제** — 최고 성능도 오라클과 24점 차이인 데다 학습 데이터가 없고 zero-dep 원칙에 어긋납니다. 정규식 카테고리 + outcome 신호 조합이 현재 위치.

출처(RouteLLM·FrugalGPT·RouterArena·라우팅 서베이 arxiv 링크)와 로컬 실측 상세 테이블, 티어 기준 전문은 [docs/TIER_CRITERIA.md](./docs/TIER_CRITERIA.md) 참고.
</details>
